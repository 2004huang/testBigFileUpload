...
@interface SIMYBPopupMenuCell : UITableViewCell
@property (nonatomic, assign) BOOL isShowSeparator;
@property (nonatomic, strong) UIColor * separatorColor;
@end

@implementation SIMYBPopupMenuCell
...
#pragma mark - ///////////
#pragma mark - SIMYBPopupMenu

@interface SIMYBPopupMenu ()
<
UITableViewDelegate,
...
- (instancetype)initWithTitles:(NSArray *)titles
                     ...
                     menuWidth:(CGFloat)itemWidth
                      delegate:(id<SIMYBPopupMenuDelegate>)delegate
{
    if (self = [super init]) {
        ...
        _offset = 0.0;
        _type = SIMYBPopupMenuTypeDefault;
        ...
        _bgView.alpha = 0;
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action: @selector(clickMargin)];
...
+ (instancetype)showAtPoint:(CGPoint)point titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth delegate:(id<SIMYBPopupMenuDelegate>)delegate
{
    SIMYBPopupMenu *popupMenu = [[SIMYBPopupMenu alloc] initWithTitles:titles icons:icons menuWidth:itemWidth delegate:delegate];
    [popupMenu showAtPoint:point];
    return popupMenu;
}

+ (instancetype)showRelyOnView:(UIView *)view titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth delegate:(id<SIMYBPopupMenuDelegate>)delegate
{
    SIMYBPopupMenu *popupMenu = [[SIMYBPopupMenu alloc] initWithTitles:titles icons:icons menuWidth:itemWidth delegate:delegate];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    SIMYBPopupMenuCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (!cell) {
        cell = [[SIMYBPopupMenuCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:identifier];
    }
    cell.backgroundColor = [UIColor clearColor];
    cell.textLabel.textColor = _textColor;
    cell.textLabel.font = FontRegularName(_fontSize);
...
{
    SIMYBPopupMenuCell *cell = [self getLastVisibleCell];
...
- (SIMYBPopupMenuCell *)getLastVisibleCell
{
...
- (void)setType:(SIMYBPopupMenuType)type
{
    _type = type;
    switch (type) {
        case SIMYBPopupMenuTypeDark:
        {
