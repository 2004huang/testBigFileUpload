...
@interface SIMOptionView ()<UITableViewDelegate,UITableViewDataSource>
...
- (void)setUI {
    ...
    self.cornerRadius = 5;
    self.borderWidth = 1;
    self.borderColor = ZJYColorHex(@"#ebebeb");
...
- (void)show {
    ...
    NSString *string = [self.dataSource[self.dataSource.count - 1] optionKey];
    CGSize tableallSize = [string sizeWithAttributes:@{NSFontAttributeName:FontRegularName(13)}];
    CGFloat tableSize = tableallSize.width+50;
//    NSLog(@"stringstringstring %@",string);
//    tableViewFrame.size.width = frame.size.width;
    tableViewFrame.size.width = tableSize;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SIMOptionList *person = self.dataSource[indexPath.row];
    cell.textLabel.text = person.optionKey;
    cell.textLabel.font = FontRegularName(13);
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    SIMOptionList *person = self.dataSource[indexPath.row];
    NSRange range = [person.optionKey rangeOfString:@" "];
    if (range.location != NSNotFound) {
        self.title = [person.optionKey substringToIndex:range.location];//截取范围内的字符串
    }
//    NSLog(@"optionView.title22 %@",self.title);
    [self dismiss];
    if ([self.delegate respondsToSelector:@selector(optionView:selectedModel:)]) {
        [self.delegate optionView:self selectedModel:person];
    }
...
- (void)setTitleFontSize:(CGFloat)titleFontSize {
    ...
    self.titleLabel.font = FontRegularName(titleFontSize);
}
