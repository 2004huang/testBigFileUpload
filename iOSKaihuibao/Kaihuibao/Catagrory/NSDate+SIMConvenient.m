//
//  NSDate+SIMConvenient.m
//  Tictalk
//
//  Created by Ferris on 2017/1/6.
//  Copyright © 2017年 Tictalkin. All rights reserved.
//
...
@implementation NSDate (SIMConvenient)
 ...
 */
+ (NSDate *) dateTomorrow {
    return [self convertDateToLocalTime:[NSDate dateWithDaysFromNow:0]];
}
...
-(NSString *)commentDateStringToShow
{
    if([self isToday])
    {
    ...
    {
        NSInteger days = [self daysBeforeDate:[NSDate date]];
        return [NSString stringWithFormat:@"%ld天",(long)days];
    }
}

+ (NSString *)dateTransform:(NSDate *)date {
    ...
    NSString *dateHour = [inputFormatter stringFromDate:date];
    NSLog(@"ceshidatehahahdatedate %@",dateHour);
