...
- (void)defaultMore
{
    ...
    picture.title = SIMLocalizedString(@"IMNewChatOtherOneTitle", nil);
    picture.path = TUIKitResource(@"more_picture");
    [moreMenus addObject:picture];

    TMoreCellData *camera = [[TMoreCellData alloc] init];
    camera.title = SIMLocalizedString(@"IMNewChatOtherTwoTitle", nil);
    camera.path = TUIKitResource(@"more_camera");
    [moreMenus addObject:camera];

    TMoreCellData *video = [[TMoreCellData alloc] init];
    video.title = SIMLocalizedString(@"IMNewChatOtherThreeTitle", nil);
    video.path = TUIKitResource(@"more_video");
    [moreMenus addObject:video];

    TMoreCellData *file = [[TMoreCellData alloc] init];
    file.title = SIMLocalizedString(@"IMNewChatOtherFourTitle", nil);
    file.path = TUIKitResource(@"more_file");
    [moreMenus addObject:file];
    
    TMoreCellData *loacate = [[TMoreCellData alloc] init];
    loacate.title = SIMLocalizedString(@"IMNewChatOtherFiveTitle", nil);
    loacate.path = TUIKitResource(@"more_location");
    [moreMenus addObject:loacate];
