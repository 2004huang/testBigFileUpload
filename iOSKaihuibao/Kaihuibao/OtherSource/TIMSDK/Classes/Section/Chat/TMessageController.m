            ...
            if([imMsg respondsToLocator:[msg locator]]){
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    if([uiMsg isKindOfClass:[TImageMessageCellData class]]){
                        ...
                        [cell setData:data];
                    }else if([uiMsg isKindOfClass:[TLoactionMessageCellData class]]){
                        TLoactionMessageCellData *data = (TLoactionMessageCellData *)uiMsg;
//                        data.loacionStr = progress.intValue;
                        TLoactionMessageCell *cell = [ws.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:i inSection:0]];
                        [cell setData:data];
                    }
                });
            }
    ...
    for (NSInteger k = msgs.count - 1; k >= 0; --k) {
        ...
        for (int i = 0; i < msg.elemCount; ++i) {
            ...
            if([elem isKindOfClass:[TIMTextElem class]]){
                ...
                soundData.path = sound.path;
                NSLog(@"sound.path %@ === %@",sound.path, sound.uuid);
                data = soundData;
            }
                ...
                TIMLocationElem *loaction = (TIMLocationElem *)elem;
                TLoactionMessageCellData *loactionData = [[TLoactionMessageCellData alloc] init];
//                loactionData.content = [NSString stringWithFormat:@"纬度%f 经度%f ",loaction.latitude,loaction.longitude];
                loactionData.loacionStr = loaction.desc;
                loactionData.longitude = loaction.longitude;
                loactionData.latitude = loaction.latitude;
                data = loactionData;
            }
...
//                systemData.content = custom.ext;
                ...
                if ([[dictionary objectForKey:@"type"] isEqualToString:@"mainCall"]) {
                    ...
                    if ([[dictionary objectForKey:@"reason"] intValue] == 1) {
                        // 对方忙
                        TTextMessageCellData *textData = [[TTextMessageCellData alloc] init];
                        textData.content = msg.isSelf?@"您正忙拒绝了视频通话":@"对方忙，请稍后再联系";
                        data = textData;
                    }else {
                        // 对方取消了视频通话
                        TTextMessageCellData *textData = [[TTextMessageCellData alloc] init];
                        textData.content = msg.isSelf?@"您拒绝了视频通话":@"对方拒绝了视频通话";
                        data = textData;
                    }
            ...
            if(data){
                ...
                if([[msg getConversation] getType] == TIM_GROUP){
                    ...
                    NSLog(@"headfaceURL %@",profile.faceURL);
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([data isKindOfClass:[TTextMessageCellData class]]){
        ...
        TLoactionMessageCellData *file = _uiMsgs[indexPath.row];
        TLoactionMessageCell *cell = [[TLoactionMessageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:TLoactionMessageCell_ReuseId];
        height = [cell getHeight:file];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if([data isKindOfClass:[TTextMessageCellData class]]){
        ...
        cell = [tableView dequeueReusableCellWithIdentifier:TLoactionMessageCell_ReuseId];
        if(!cell){
            cell = [[TLoactionMessageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:TLoactionMessageCell_ReuseId];
            cell.delegate = self;
        }
    }
...
- (void)sendMessage:(TMessageCellData *)msg
{
    ...
    [self getLastDisplayString:imMsg withNickName:msg.name];
    __weak typeof(self) ws = self;
    [conv sendMessage:imMsg succ:^{
        ...
        NSLog(@"sendmessagesuccesscode");
    } fail:^(int code, NSString *desc) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [ws changeMsg:msg status:Msg_Status_Fail];
        });
        NSLog(@"sendmessagefailcode%d %@",code,desc);
    }];
...
- (NSString *)getLastDisplayString:(TIMMessage *)msg withNickName:(NSString *)nickName;
{
    NSString *str = @"";
    for (int i = 0; i < msg.elemCount; ++i) {
        TIMElem *elem = [msg getElem:i];
    ...
    offlineInfo.desc = [NSString stringWithFormat:@"%@:%@",self.currentUser.nickname,str];
    offlineInfo.ext = [NSString stringWithFormat:@"%@",str];
...
- (TIMMessage *)transIMMsgFromUIMsg:(TMessageCellData *)data
{
    ...
    if([data isKindOfClass:[TTextMessageCellData class]]){
        ...
        TLoactionMessageCellData *uiLocation = (TLoactionMessageCellData *)data;
        imLocation.desc = uiLocation.loacionStr;
        imLocation.longitude = uiLocation.longitude;
        imLocation.latitude = uiLocation.latitude;
...
- (NSString *)getDateDisplayString:(NSDate *)date{
    ...
    if (nowCmps.day==myCmps.day) {
        ...
        dateFmt.AMSymbol = SIMLocalizedString(@"IMDateAM", nil);
        dateFmt.PMSymbol = SIMLocalizedString(@"IMDatePM", nil);
        dateFmt.dateFormat = [NSString stringWithFormat:@"%@ aaahh:mm",SIMLocalizedString(@"TimeYesterdayAndTimeList", nil)];
        ...
        if ((nowCmps.day-myCmps.day) <=7) {
            switch (comp.weekday) {
                case 1:
                    dateFmt.dateFormat = [NSString stringWithFormat:@"%@ aaahh:mm",SIMLocalizedString(@"IMDateSun", nil)];
                    break;
                case 2:
                    dateFmt.dateFormat = [NSString stringWithFormat:@"%@ aaahh:mm",SIMLocalizedString(@"IMDateMon", nil)];
                    break;
                case 3:
                    dateFmt.dateFormat = [NSString stringWithFormat:@"%@ aaahh:mm",SIMLocalizedString(@"IMDateTus", nil)];
                    break;
                case 4:
                    dateFmt.dateFormat = [NSString stringWithFormat:@"%@ aaahh:mm",SIMLocalizedString(@"IMDateWen", nil)];
                    break;
                case 5:
                    dateFmt.dateFormat = [NSString stringWithFormat:@"%@ aaahh:mm",SIMLocalizedString(@"IMDateThur", nil)];
                    break;
                case 6:
                    dateFmt.dateFormat = [NSString stringWithFormat:@"%@ aaahh:mm",SIMLocalizedString(@"IMDateFri", nil)];
                    break;
                case 7:
                    dateFmt.dateFormat = [NSString stringWithFormat:@"%@ aaahh:mm",SIMLocalizedString(@"IMDateSat", nil)];
                    break;
                default:
                    break;
            }
...
- (void)sendLocationMessage:(NSDictionary *)loactionDic
{
    TLoactionMessageCellData *loactionData = [[TLoactionMessageCellData alloc] init];
    loactionData.loacionStr = loactionDic[@"locationStr"];
    loactionData.latitude = [loactionDic[@"latitude"] doubleValue];
    loactionData.longitude = [loactionDic[@"longitude"] doubleValue];
    loactionData.head = self.currentUser.avatar;
    loactionData.isSelf = YES;
    [self sendMessage:loactionData];
}
- (void)sendImageMessage:(UIImage *)image;
{
    ...
    uiImage.head = self.currentUser.avatar;
    uiImage.isSelf = YES;
...
- (void)sendVideoMessage:(NSURL *)url
{
    ...
    uiVideo.head = self.currentUser.avatar;
    uiVideo.isSelf = YES;
...
- (void)sendFileMessage:(NSURL *)url
{
    ...
    [coordinator coordinateReadingItemAtURL:url options:0 error:&error byAccessor:^(NSURL *newURL) {
        ...
        if([[NSFileManager defaultManager] fileExistsAtPath:filePath]){
            ...
            uiFile.head = self.currentUser.avatar;
            uiFile.isSelf = YES;
