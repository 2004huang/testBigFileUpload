...
- (void)defaultLayout
{
    ...
    _title.text = SIMLocalizedString(@"IMChatGesCancel", nil);
...
- (void)setStatus:(RecordStatus)status
{
    switch (status) {
        ...
        {
            _title.text = SIMLocalizedString(@"IMChatGesRelease", nil);
            _title.backgroundColor = Record_Title_Background_Color;
            break;
        }
        case Record_Status_TooShort:
        {
            _title.text = SIMLocalizedString(@"IMChatGesTooShort", nil);
            _title.backgroundColor = [UIColor clearColor];
            break;
        }
        case Record_Status_TooLong:
        {
            _title.text = SIMLocalizedString(@"IMChatGesTooLong", nil);
            _title.backgroundColor = [UIColor clearColor];
            break;
        }
        default:
