...
{
    _data = data;
    if ([_data.head isEqualToString:TUIKitResource(@"default_head")]) {
        _headImageView.image = [UIImage imageNamed:_data.head];
    }else {
        [_headImageView sd_setImageWithURL:[NSURL URLWithString:[NSString iconGetString:_data.head]] placeholderImage:[UIImage imageNamed:@"avatar"] options:SDWebImageAllowInvalidSSLCertificates];
    }
...
//    _headImageView.backgroundColor = self.backgroundColor;
    ...
    _timeLabel.font = FontRegularName(12);
    _timeLabel.textColor = [UIColor lightGrayColor];
    _timeLabel.layer.masksToBounds = YES;
    [self addSubview:_timeLabel];

    _titleLabel = [[UILabel alloc] init];
    _titleLabel.font = FontRegularName(16);
    ...
    _subTitleLabel.font = FontRegularName(14);
