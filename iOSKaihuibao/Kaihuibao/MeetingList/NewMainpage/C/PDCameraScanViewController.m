...
AVCaptureMetadataOutputObjectsDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,CLConferenceDelegate> //遵守AVCaptureMetadataOutputObjectsDelegate协议
...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *photoBtn = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"scanErweima_barbutton"] style:UIBarButtonItemStylePlain target:self action:@selector(choicePhoto)];
    self.navigationItem.rightBarButtonItem = photoBtn;
    
    //定位扫描框在屏幕正中央，并且宽高为200的正方形
    self.scanView = [[UIView alloc]initWithFrame:CGRectMake((screen_width-200)/2, (screen_height - StatusNavH - 200)/2, 200, 200)];
    [self.view addSubview:self.scanView];
    
    //设置扫描界面（包括扫描界面之外的部分置灰，扫描边框等的设置）,后面设置
    PDCameraScanView *clearView = [[PDCameraScanView alloc]initWithFrame:CGRectMake(0, 0, screen_width, screen_height - StatusNavH)];
    [self.view addSubview:clearView];
    
    //初始化并启动扫描
    [self startScan];
    
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(20, (screen_height - StatusNavH - 200)/2 + 240, screen_width - 40, 40)];
    label.textColor = [UIColor whiteColor];
    label.font = FontRegularName(14);
    label.textAlignment = NSTextAlignmentCenter;
    label.text = @"将取景框对准视频终端二维码";
...
//    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:object.stringValue]];
    ...
    NSRange range = [urlStr rangeOfString:@"m="];
    NSString *tempStr;
    if (range.length > 0  ||  range.location != NSNotFound) {
        tempStr = [urlStr substringFromIndex:range.location+range.length];
        NSLog(@"扫描cidstringcidstring %@",tempStr);
        if (tempStr.length > 0) {
            
            [SIMNewEnterConfTool enterTheMineConfWithCid:tempStr psw:@"" confType:EnterConfTypeConf isJoined:NO viewController:self];
        }else {
            [MBProgressHUD cc_showText:@"二维码入会失败，请稍后再试"];
        }
    }else {
        [MBProgressHUD cc_showText:@"二维码入会失败，请稍后再试"];
    }
...
-(void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    CIQRCodeFeature *feature = (CIQRCodeFeature *)[detector featuresInImage:ciImage options:nil].firstObject;
...
//    }
    [self dismissViewControllerAnimated:YES completion:^{
        if (feature.messageString) {
            NSString *urlStr = feature.messageString;// 这个就是我们想要的值
            NSLog(@"识别图片结果cidstringcidstring %@",urlStr);
            NSRange range = [urlStr rangeOfString:@"m="];
            NSString *tempStr;
            if (range.length > 0  ||  range.location != NSNotFound) {
                tempStr = [urlStr substringFromIndex:range.location+range.length];
                NSLog(@"扫描图片cidstringcidstring %@",tempStr);
                if (tempStr.length > 0) {
            ...
            [MBProgressHUD cc_showText:@"请选择要识别的二维码"];
        }
